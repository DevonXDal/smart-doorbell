...
class MockWatcher:
    def __init__(self, server_handler: MainServerHandler):
        self._mock_cam = cv2.VideoCapture(0) # Requires the testing device to possess a webcam of similar form. DroidCam cam can serve as a temporary if necessary
        keyboard.add_hotkey('ctrl+alt+p', lambda: self._handle_picture_and_send())
        self.server_handler = server_handler

    def end_program(self):
        self._mock_cam.release()
